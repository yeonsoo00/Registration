"""Grouped affine inference: one matrix is applied to every stain in a group.

python inference.py \
    --checkpoint /home/yec23006/projects/research/Registration/Grouped/ckpt/group_stack_fixed/stage2_best_model.pt \
    ...
    --output_dir /home/yec23006/projects/research/Registration/Grouped/Results/group_stack_structural_rgb_stage2 \
...
def main(a):
    ...
    ds = CartilageDataset(
        ...
        group_input_mode=pre["group_input_mode"],
        include_group1=pre.get("include_group1", True),
        enhance_groups=tuple(pre.get("enhance_groups", [])),
        enhancement_strength=pre.get("enhancement_strength", 0.7),
        enhancement_mode=pre.get("enhancement_mode", "legacy"),
        trap_enhancement_strength=pre.get("trap_enhancement_strength", 0.8),
        cfo_enhancement_strength=pre.get("cfo_enhancement_strength", 0.5),
        sfo_enhancement_strength=pre.get("sfo_enhancement_strength", 1.0),
        sfo_hue_range_degrees=tuple(pre.get("sfo_hue_range_degrees", [70.0, 200.0])),
        trap_morphology_kernel_size=pre.get("trap_morphology_kernel_size", 3),
        trap_morphology_iterations=pre.get("trap_morphology_iterations", 1),
    )
    dl = DataLoader(
        ds,
    ...
    with torch.no_grad():
        for batch in dl:
            gi = batch["group_input"].to(device)
            fixed = batch["fixed_mineral"].to(device)
            group = batch["group"].to(device)
            params = model(gi, fixed, group)
    ...
    print(
        f"Saved {len(rows)} aligned stain images and group overlays to {a.output_dir}"
    )
